...
using IRM.WinForm.Inventory.MotorcycleColor;
...
public partial class FrmMotorcycleColorView : Form
{
    private CancellationTokenSource? _cts;
    private Guid ColorId { get; set; }
    private BindingList<MotorcycleColorDto> _colorsList;
    private readonly MotorcycleColorServices _colorServices;
    public FrmMotorcycleColorView(MotorcycleColorServices colorServices)
    {
        InitializeComponent();
        SetColorUI();
    ...
    private void FrmItemColor_Load(object sender, EventArgs e)
    {
    ...
    private async void InvokeGetByIdAsync(Guid id)
    {
        try
        {
            ...
            var data = await _colorServices.GetByIdAsync(id);
    ...
    private void InvokeDataViewSetting()
    {
        try
        {
            ...
            // Set custom columns
            MotorcycleColorFormatingDGColumns.Apply(DataGridView);
        }
        catch (Exception ex)
        {
    ...
    private async void InvokeFilterAsync()
    {
        ...
        var result = await _colorServices.FilterAsync();
        _colorsList = new BindingList<MotorcycleColorDto>(result);

        if (_colorsList.Count > 0)
        {
            DataGridView.DataSource = _colorsList;
        }
    ...
    private async void BtnPersistence_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var newDto = new MotorcycleColorDto
            {
                Id = ColorId,
                Name = TxtName.Text.Trim(),
                IsActive = CBIsActive.Checked,
            };
            ColorId = await _colorServices.PersistenceAsync(newDto);
            newDto.Id = ColorId;

            MotorcycleColorUpdatingMemoryData.Excecute(newDto, _colorsList);
            DataGridView.DataSource = _colorsList;
            ClearFields();
        }
    ...
    private async void DataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
    {
        ...
        ColorId = Guid.Parse(DataGridView.Rows[e.RowIndex].Cells["Id"].Value!.ToString()!);

        if (DataGridView.Columns[e.ColumnIndex].Name == "editCol")
        {
            InvokeGetByIdAsync(ColorId);
        }
        else if (DataGridView.Columns[e.ColumnIndex].Name == "deleteCol")
        {
            if (MessageBox.Show("¿Seguro que desea borrar el registro?... No se recueperará!", "Eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    _cts = new CancellationTokenSource();
                    await _colorServices.DeleteAsync(ColorId);
